...
#define RAW_COPS_DEVICE "/dev/ttyS5"
#define RAW_COPS_BAUD B115200
#define RAW_COPS_ATTEMPTS 3
#define RAW_COPS_TIMEOUT_MS 5000
#define RAW_COPS_RESPONSE_SIZE 512
...
static int configure_serial(int fd)
{
    ...
    cfmakeraw(&options);
    cfsetispeed(&options, RAW_COPS_BAUD);
    cfsetospeed(&options, RAW_COPS_BAUD);
...
{
    static const char command[] = "AT+COPS?\r\n";
    unsigned char response[RAW_COPS_RESPONSE_SIZE];
    size_t response_length = 0;
    int fd;
    int remaining_ms = RAW_COPS_TIMEOUT_MS;

    printf("Attempt %d: opening %s\n", attempt, RAW_COPS_DEVICE);

    fd = open(RAW_COPS_DEVICE, O_RDWR | O_NOCTTY | O_NONBLOCK);
    if (fd < 0)
    {
    ...
    printf("Attempt %d: sending AT+COPS?\\r\\n\n", attempt);
...
int main(void)
{
    ...
    for (attempt = 1; attempt <= RAW_COPS_ATTEMPTS; attempt++)
    {
        ...
        if (attempt < RAW_COPS_ATTEMPTS)
        {
            sleep(1);
        }
    }

    printf("Raw COPS test complete: %d/%d attempt(s) received data\n", responses, RAW_COPS_ATTEMPTS);
